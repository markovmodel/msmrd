...
def run_mfpts_from_bath(state, runs):
    ...
    pickle.dump(np.array(fpts), open('../data/asym2D/MFPTS/bath_'+str(state)+'_'+str(runs)+'runs_dt1E-4_SF0.7.p', 'wa'))
...
def run_mfpts_to_bath(state, runs):
    ...
    pickle.dump(np.array(fpts), open('../data/asym2D/MFPTS/'+str(state)+'_bath_'+str(runs)+'runs_dt1E-4_SF0.7.p', 'wa'))
    print 'state '+str(state)+' done'
    return np.mean(fpts)
...
#MFPT_from_bath = pool.map(partial(run_mfpts_from_bath, runs=100000), states)

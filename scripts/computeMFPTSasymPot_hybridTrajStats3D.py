...
# Hybrid simulation code to obtain MFPTs between discrete states and to and from the bath state
# Note this scipt does not use an MSM, the notation with MSM is left for
...
def run_mfpts(statePair, runs, dt=0.01):
    ...
    for run in range(runs):
        ...
        integrator.clock = 0.
        integrator.MSMstate = statePair[0]
...
def run_mfpts_to_bath(state, runs, dt = 0.01):
    ...
    for run in range(runs):
        integrator.particle.position = np.zeros(3)
        integrator.clock = 0.
        integrator.MSMstate = state
        integrator.lastState = state
        integrator.lastStateTime = 0
        integrator.transition = True
        integrator.MSMactive = True
        fpts.append(sim.run_mfpt(3.0))
    pickle.dump(np.array(fpts), open('../data/asym3D/MFPTS/hybrid/'+str(state)+'_to_bath_'+str(runs)+'runs_hybridtrajStats_box_dt' + dt + 'exitCompensation.p', 'wa'))
    return np.mean(fpts)
